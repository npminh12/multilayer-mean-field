...
class nnet(object):
    def __init__(self, NN_architecture):
        ...
        self.element_size = NN_architecture['element_size']  
        self.element_init_mode = NN_architecture['element_init_mode']  
    ...
    def _weight_init(self, layer):
        ...
        if self._layer_type(layer)=='conv':                
            ...
            if layer==1:
                dims[1] = dims[1]*self._element_size()
            elif layer==self.depth:
                dims[0] = dims[0]*self._element_size()
            else:
                dims = dims*self._element_size()
    ...
    def _bias_init(self, layer):
        ...
        return tf.random_normal([channel_out*self._element_size()], mean=mean, stddev=std, dtype=self.dtype)
    
    def _element_init(self, layer):
